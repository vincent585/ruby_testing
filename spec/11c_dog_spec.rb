...
describe Dog do
  ...
  subject(:winston) { described_class.new('Winston', 'Labrador Retriever', 'black') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'When Dog has a method name shared with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(winston).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(winston).to be_sleeping
  end
end
