...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:beveragini) { described_class.new('milk') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(beveragini.type).to eq('milk')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(beveragini.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_beer) { described_class.new('beer', 2) }
