...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:five_and_nine) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        solution = five_and_nine.make_guess
        expect(solution).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:eight_and_nine) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        solution = eight_and_nine.make_guess
        expect(solution).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      ...
      it 'returns 1' do
        solution = zero_three.make_guess
        expect(solution).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:both_three) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        solution = both_three.make_guess
        expect(solution).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:correct_guess) { described_class.new(0, 9, random_number, 6) }
      ...
      it 'is game over' do
        expect(correct_guess).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:incorrect_guess) { described_class.new(0, 9, random_number, 2) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'does not update max' do
        low_guess_game.update_range
        same_max = low_guess_game.max
        expect(same_max).to eq(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        ...
        same_min = high_guess_game.min
        expect(same_min).to eq(0)
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        new_min = close_guess_game.min
        same_max = close_guess_game.max
        expect(new_min).to eq(same_max)
      end

      it 'does not update max' do
        close_guess_game.update_range
        same_max = close_guess_game.max
        expect(same_max).to eq(8)
      end
    end

    context 'when the guess is outside the range' do
      subject(:out_of_range_guess) { described_class.new(0, 9, number_range, -3) }
      it 'does not update min nor max' do
        out_of_range_guess.update_range
        og_min = out_of_range_guess.min
        og_max = out_of_range_guess.max
        new_min = out_of_range_guess.guess + 1
        new_max = out_of_range_guess.guess - 1
        expect(og_min).not_to eq(new_min)
        expect(og_max).not_to eq(new_max)
      end
    end
  end
end
