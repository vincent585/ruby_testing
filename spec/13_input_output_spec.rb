...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:incorrect_guess) { described_class.new(5, '1') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        phrase = 'gabba gool'
        valid_input = '2'
        allow(game_loop).to receive(:player_input).and_return(letter, phrase, valid_input)
      end

      it 'completes loop and displays error message twice' do
        expect(game_loop).to receive(:puts).with('Input error!').twice
        2.times { game_loop.player_turn }
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_four_plus) { described_class.new(6, '6', 5) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        correct_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_four_plus.final_message }.to output(correct_phrase).to_stdout
      end
    end
  end
end
